...
final class GitPatchToolExecutorTests: XCTestCase {
    ...
    func testUnstageHunkPreservesWorkingTreeChange() throws {
        let root = try makeTempDirectory()
        try initializeGitRepo(at: root)
        let file = root.appendingPathComponent("hello.txt")
        try "one\ntwo\nthree\n".write(to: file, atomically: true, encoding: .utf8)
        ...
        +TWO
         three
        """
        XCTAssertTrue(GitToolExecutor().stageHunk(cwd: root, path: "hello.txt", patch: patch).ok)

        let result = GitToolExecutor().unstageHunk(cwd: root, path: "hello.txt", patch: patch)

        XCTAssertTrue(result.ok, "\(result.error ?? "") \(result.stderr)")
        XCTAssertEqual(GitToolExecutor().diff(cwd: root, staged: true).stdout, "")
        XCTAssertTrue(GitToolExecutor().diff(cwd: root).stdout.contains("+TWO"))
