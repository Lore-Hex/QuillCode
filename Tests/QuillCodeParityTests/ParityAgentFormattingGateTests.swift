...
final class ParityAgentFormattingGateTests: QuillCodeParityTestCase {
    func testAgentRunnerDelegatesFinalAnswerFormatting() throws {
        ...
        let shellFormatterText = try Self.agentSourceText(
            named: "AgentShellToolAnswerFormatters.swift"
        )
        let browserFormatterText = try Self.agentSourceText(
            named: "AgentBrowserToolAnswerFormatters.swift"
        )
        let gitFormatterText = try Self.agentSourceText(
            named: "AgentGitToolAnswerFormatters.swift"
        )
        let utilityFormatterText = try Self.agentSourceText(
            named: "AgentUtilityToolAnswerFormatters.swift"
        )
        ...
        Self.assertSource(
            gitFormatterText,
            contains: "ToolDefinition.gitPullRequestReviewThreads.name"
        )
