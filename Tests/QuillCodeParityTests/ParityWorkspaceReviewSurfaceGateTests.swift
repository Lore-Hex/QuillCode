...
final class ParityWorkspaceReviewSurfaceGateTests: QuillCodeParityTestCase {
    func testWorkspaceSurfaceDelegatesReviewSurfaceContracts() throws {
        ...
        let scopeText = try Self.appSourceText(named: "WorkspaceReviewScope.swift")
        let fileText = try Self.appSourceText(named: "WorkspaceReviewFileSurface.swift")
        let commentText = try Self.appSourceText(named: "WorkspaceReviewCommentSurface.swift")
        let actionText = try Self.appSourceText(named: "WorkspaceReviewActionSurface.swift")

        XCTAssertTrue(reviewText.contains("public struct WorkspaceReviewSurface"), "Review aggregate should live in its focused contract file.")
        XCTAssertTrue(reviewText.contains("public var activeScope"), "Review surfaces should identify the diff scope they render.")
        XCTAssertTrue(scopeText.contains("public enum WorkspaceReviewScope"), "Review scope should live in its focused typed contract file.")
        XCTAssertTrue(fileText.contains("public func actions(in scope"), "Review rows should derive actions from their active scope.")
        XCTAssertTrue(fileText.contains("public struct WorkspaceReviewFileSurface"), "Review file rows should live in their focused contract file.")
        XCTAssertTrue(fileText.contains("public struct WorkspaceReviewHunkSurface"), "Review hunk rows should live beside file diff rows.")
        XCTAssertTrue(fileText.contains("public struct WorkspaceReviewLineSurface"), "Review line rows should live beside file diff rows.")
        XCTAssertTrue(fileText.contains("public enum WorkspaceReviewLineKind"), "Review line kinds should live beside file diff rows.")
        XCTAssertTrue(fileText.contains("public var unreadableReason"), "Review files should own unreadable-file presentation policy.")
        XCTAssertTrue(fileText.contains("isDeleted"), "Review files should carry deleted-file state for action availability.")
        XCTAssertTrue(commentText.contains("public struct WorkspaceReviewCommentSurface"), "Review comments should live in their focused contract file.")
        XCTAssertTrue(actionText.contains("public struct WorkspaceReviewActionSurface"), "Review actions should live in their focused contract file.")
        XCTAssertTrue(actionText.contains("public enum WorkspaceReviewActionKind"), "Review action kinds should live beside review actions.")
        XCTAssertFalse(reviewText.contains("public enum WorkspaceReviewScope"), "Review aggregate should not own scope implementation.")
        XCTAssertFalse(reviewText.contains("public struct WorkspaceReviewFileSurface"), "Review aggregate should not own file-row implementation.")
        XCTAssertFalse(reviewText.contains("public struct WorkspaceReviewCommentSurface"), "Review aggregate should not own comment implementation.")
        XCTAssertFalse(reviewText.contains("public struct WorkspaceReviewActionSurface"), "Review aggregate should not own action implementation.")
    ...
    func testWorkspaceModelDelegatesReviewActionToolCallPlanning() throws {
        ...
        XCTAssertFalse(modelText.contains("ToolDefinition.gitUnstageHunk.name"), "WorkspaceModel should not own hunk unstage details.")
