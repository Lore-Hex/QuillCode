...
final class ParityWorkspaceSurfaceGateTests: QuillCodeParityTestCase {
    ...
    func testNativeSecondaryPanesUseFocusedViewFiles() throws {
        ...
        XCTAssertTrue(extensionCardsText.contains("ProjectExtensionManifestSurface"), "Extension cards should own manifest row display.")
        XCTAssertTrue(extensionCardsText.contains("extensionCommand(id:"), "Extension cards should own row action command construction.")
        XCTAssertTrue(extensionMetadataText.contains("MCPToolDescriptor"), "MCP extension metadata display should stay in focused Extensions metadata UI.")
        XCTAssertTrue(extensionMetadataText.contains("probeReferenceActionGroup"), "MCP resource and prompt actions should be grouped in focused metadata UI.")
        ...
        XCTAssertFalse(extensionsText.contains("private func extensionCard"), "Extensions pane shell should not own extension-card internals.")
        XCTAssertFalse(extensionsText.contains("probeMetadataChips"), "Extensions pane shell should not own MCP metadata internals.")
    ...
    func testComposerSeparatesModelAndApprovalModeControls() throws {
        ...
        XCTAssertTrue(htmlTranscriptText.contains(#"testID: "model-picker-button""#), "HTML composer should expose a model control.")
        XCTAssertTrue(htmlTranscriptText.contains(#"testID: "mode-picker-button""#), "HTML composer should expose a separate mode control.")
