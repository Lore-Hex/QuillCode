...
final class ParityWorkspaceAutomationTerminalIntegrationGateTests: QuillCodeParityTestCase {
    func testWorkspaceAutomationIntegrationTestsOwnModelAutomationFlows() throws {
        ...
        let automationCommandTests = try Self.appTestSourceText(
            named: "WorkspaceAutomationIntegrationTests.swift"
        )
        let automationSchedulingTests = try Self.appTestSourceText(
            named: "WorkspaceAutomationSchedulingIntegrationTests.swift"
        )
        let automationRunTests = try Self.appTestSourceText(
            named: "WorkspaceAutomationRunIntegrationTests.swift"
        )
        let automationSupport = try Self.appTestSourceText(
            named: "WorkspaceAutomationIntegrationTestSupport.swift"
        )
    ...
    func testWorkspaceTerminalIntegrationTestsOwnModelTerminalFlows() throws {
        let modelTests = try Self.appTestSourceText(named: "WorkspaceModelTests.swift")
        let terminalIntegrationTests = try Self.appTestSourceText(
            named: "WorkspaceTerminalIntegrationTests.swift"
        )

        Self.assertSource(terminalIntegrationTests, containsAll: [
