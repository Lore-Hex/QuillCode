...
final class ParityCoreModelGateTests: QuillCodeParityTestCase {
    func testCoreToolModelsLiveOutsideGeneralDomainModels() throws {
        ...
        let browserScriptOutputText = try Self.coreSourceText(named: "BrowserScriptToolOutput.swift")
        ...
        XCTAssertTrue(browserScriptOutputText.contains("public struct BrowserScriptToolOutput"), "Browser script output compatibility belongs in its own focused core file.")
        ...
        XCTAssertTrue(definitionsText.contains("static let browserScript"), "Browser script tool schema should live in the focused catalog.")
        ...
        XCTAssertFalse(modelsText.contains("public struct BrowserScriptToolOutput"), "General domain models should not own tool-specific output compatibility.")
