...
final class ParityWorkspaceUIStateGateTests: QuillCodeParityTestCase {
    func testWorkspaceModelDelegatesUIStateContracts() throws {
        ...
        let sendLifecycleText = try Self.appSourceText(
            named: "WorkspaceComposerSendLifecycle.swift"
        )
        let sendStartText = try Self.appSourceText(
            named: "WorkspaceAgentSendStartPlanner.swift"
        )
        let sendProgressText = try Self.appSourceText(
            named: "WorkspaceAgentSendProgressPlanner.swift"
        )
        let sendTerminalText = try Self.appSourceText(
            named: "WorkspaceAgentSendTerminalPlanner.swift"
        )
        ...
        Self.assertSource(
            sendLifecycleText,
            contains: "enum WorkspaceComposerSendLifecycle"
        )
        ...
        Self.assertSource(
            modelText,
            contains: "public internal(set) var composer: ComposerState"
        )
