...
final class ParityBrowserGateTests: QuillCodeParityTestCase {
    func testWorkspaceModelDelegatesBrowserSurfaceTypes() throws {
        ...
        XCTAssertTrue(browserStateText.contains("public struct BrowserState"), "Browser state should live in a focused state file.")
        XCTAssertTrue(browserTabStateText.contains("public struct BrowserTabState"), "Browser tab state should live in a focused state file.")
        XCTAssertTrue(browserSnapshotStateText.contains("public struct BrowserSnapshotState"), "Browser snapshot state should live in a focused state file.")
        XCTAssertTrue(browserCommentStateText.contains("public struct BrowserCommentState"), "Browser comment state should live in a focused state file.")
        ...
        XCTAssertFalse(browserSurfaceText.contains("public struct BrowserState"), "Browser presentation surfaces should not own browser reducer state.")
        XCTAssertFalse(browserStateText.contains("public struct BrowserSurface"), "Browser state should not own browser presentation surfaces.")
    ...
    func testVisibleBrowserSessionSyncStaysBehindSnapshotContract() throws {
        ...
        XCTAssertTrue(snapshotText.contains("public struct BrowserSessionUpdate"), "Visible browser reverse sync should have a shared app-layer update.")
        XCTAssertTrue(snapshotText.contains("public struct BrowserSessionTabUpdate"), "Visible browser tab updates should stay app-layer data.")
        ...
        XCTAssertTrue(engineText.contains("static func applySessionUpdate"), "Browser state should apply visible-session updates in the focused engine.")
        XCTAssertTrue(workflowText.contains("WorkspaceBrowserEngine.applySessionUpdate"), "Browser workflow should delegate visible-session update application.")
        XCTAssertTrue(browserModelText.contains("public func applyBrowserSessionUpdate"), "WorkspaceModel browser API should expose reverse session sync without desktop types.")
        XCTAssertTrue(presenterText.contains("var onSessionUpdate"), "Desktop presenter should report visible browser navigation through an app-layer update callback.")
        XCTAssertTrue(presenterText.contains("emitSessionUpdate()"), "Desktop presenter should emit updates when visible-session state changes.")
        ...
        XCTAssertTrue(coordinatorText.contains("installSessionUpdateHandler"), "Desktop browser coordinator should install reverse session sync handlers.")
        XCTAssertTrue(coordinatorText.contains("model.applyBrowserSessionUpdate(update)"), "Desktop browser coordinator should apply visible-session updates through the model API.")
        ...
        XCTAssertTrue(controllerText.contains("browserCoordinator.installSessionUpdateHandler"), "Desktop controller should connect visible-session reverse sync at startup.")
        ...
        XCTAssertTrue(snapshotTests.contains("testSessionUpdateRejectsUnknownActiveTab"), "Session update fallback behavior should have focused unit coverage.")
        XCTAssertTrue(engineTests.contains("testSessionUpdateSyncsVisibleBrowserNavigationBackIntoSelectedTab"), "Reverse browser session state updates should have focused engine coverage.")
        XCTAssertTrue(integrationTests.contains("testVisibleBrowserSessionUpdateRefreshesModelAndSurface"), "Reverse browser session model orchestration should have focused integration coverage.")
    }
