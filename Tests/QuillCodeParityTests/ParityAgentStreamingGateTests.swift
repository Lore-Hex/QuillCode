...
final class ParityAgentStreamingGateTests: QuillCodeParityTestCase {
    func testAgentStreamingHelpersLiveOutsideAgentRunnerFile() throws {
        ...
        let textRunnerText = try Self.agentSourceText(
            named: "AgentTextStreamActionRunner.swift"
        )
        let usageRunnerText = try Self.agentSourceText(
            named: "AgentUsageStreamActionRunner.swift"
        )
        let rawCollectorText = try Self.agentSourceText(
            named: "AgentRawTextStreamActionCollector.swift"
        )
        let textCollectorText = try Self.agentSourceText(
            named: "AgentTextStreamActionCollector.swift"
        )
        let usageCollectorText = try Self.agentSourceText(
            named: "AgentUsageStreamActionCollector.swift"
        )
        let draftPublisherText = try Self.agentSourceText(
            named: "AgentStreamingDraftPublisher.swift"
        )
        ...
        [
            ...
            textCollectorText,
            usageCollectorText
        ].forEach { collectorText in
            Self.assertSource(collectorText, contains: "AgentActionStreamCollector.collect")
        }
    ...
    func testAgentCancellationTelemetryLivesInFocusedRecorder() throws {
        ...
        Self.assertSource(
            agentText,
            contains: "AgentCancellationRecorder.recordCancelledRun"
        )
