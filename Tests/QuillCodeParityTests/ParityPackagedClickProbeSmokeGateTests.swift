...
final class ParityPackagedClickProbeSmokeGateTests: QuillCodeParityTestCase {
    ...
    func testNativeClickProbeValidatorCLIValidatesAndWritesPackagedManifests() throws {
        ...
        let shallowExtensionsActivationReport = temporaryDirectory
        ...
        let shallowExtensionsAccessibilityFrames = temporaryDirectory
        ...
        try Self.minimalPackagedWindowAccessibilityFrameReport()
            .replacingOccurrences(
                of: "rendered Extensions with its Add control and dismissed through quillcode-extensions-close with AXPress",
                with: "AXPress changed observable controller state"
            )
            .write(to: shallowExtensionsActivationReport, atomically: true, encoding: .utf8)
        try Self.minimalPackagedWindowAccessibilityFrameReport()
            .replacingOccurrences(
                of: "rendered Memories with its Add control and dismissed through quillcode-memories-close with AXPress",
                with: "AXPress changed observable controller state"
            )
        ...
        try Self.minimalPackagedWindowAccessibilityFrameReport()
            .replacingOccurrences(
                of: "rendered Review with its scope control and dismissed through quillcode-review-close with AXPress",
                with: "AXPress changed observable controller state"
            )
        ...
        let shallowExtensionsResult = try Self.runPython(validator, arguments: [
            "frames",
            shallowExtensionsActivationReport.path,
            windowScreenshot.path,
            "--manifest",
            shallowExtensionsAccessibilityFrames.path
        ])
        XCTAssertNotEqual(shallowExtensionsResult.exitCode, 0)
        XCTAssertTrue(
            shallowExtensionsResult.output.contains(
                "command.toggle-extensions does not prove rendered controls and close-button dismissal"
            ),
            shallowExtensionsResult.output
        )

        let shallowMemoriesResult = try Self.runPython(validator, arguments: [
            "frames",
            shallowMemoriesActivationReport.path,
            windowScreenshot.path,
            "--manifest",
            shallowMemoriesAccessibilityFrames.path
        ])
        XCTAssertNotEqual(shallowMemoriesResult.exitCode, 0)
        XCTAssertTrue(
            shallowMemoriesResult.output.contains(
        ...
        let shallowReviewResult = try Self.runPython(validator, arguments: [
            "frames",
            shallowReviewActivationReport.path,
            windowScreenshot.path,
            "--manifest",
            shallowReviewAccessibilityFrames.path
        ])
        XCTAssertNotEqual(shallowReviewResult.exitCode, 0)
        XCTAssertTrue(
            shallowReviewResult.output.contains(
