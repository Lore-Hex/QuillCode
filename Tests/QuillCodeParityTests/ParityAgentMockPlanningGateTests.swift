...
final class ParityAgentMockPlanningGateTests: QuillCodeParityTestCase {
    func testMockLLMClientLivesOutsideAgentRunnerFile() throws {
        ...
        let downloadParserText = try Self.agentSourceText(
            named: "AgentDownloadRequestParser.swift"
        )
        let pullRequestPlannerText = try Self.agentSourceText(
            named: "MockPullRequestIntentPlanner.swift"
        )
        let pullRequestExtractorText = try Self.agentSourceText(
            named: "MockPullRequestArgumentExtractor.swift"
        )
        ...
        Self.assertSource(
            pullRequestPlannerText,
            excludes: "static func createArguments"
        )
    }
}
