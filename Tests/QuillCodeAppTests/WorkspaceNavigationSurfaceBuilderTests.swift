...
final class WorkspaceNavigationSurfaceBuilderTests: XCTestCase {
    func testBuildsSortedProjectsAndSidebarRows() throws {
        ...
        XCTAssertEqual(
            surface.projects.items.map { $0.actions.first(where: { $0.kind == .moveToBottom })?.isEnabled },
            [true, false]
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            surface.projects.items.last?.actions.first(where: { $0.kind == .moveToBottom })?.disabledReason,
            "Already at the bottom"
        )
