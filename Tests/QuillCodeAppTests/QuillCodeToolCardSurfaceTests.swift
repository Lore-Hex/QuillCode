...
final class QuillCodeToolCardSurfaceTests: XCTestCase {
    ...
    func testArtifactStateDerivesCargoLockPreviewMetadata() throws {
        let directory = try makeQuillCodeTestDirectory()
        let report = directory.appendingPathComponent("Cargo.lock")
        let lockText = """
        ...
        let artifact = ToolArtifactState(value: report.path)
        let preview = try XCTUnwrap(artifact.cargoLockPreview)
        let byteSizeLabel = try XCTUnwrap(ToolArtifactByteSizeFormatter.label(for: XCTUnwrap(lockText.data(using: .utf8)).count))

        XCTAssertEqual(artifact.documentPreview?.kind, .data)
        XCTAssertEqual(artifact.documentPreview?.extensionLabel, "CARGO-LOCK")
        XCTAssertEqual(preview.packageCount, 3)
        XCTAssertEqual(preview.versionedPackageCount, 3)
        XCTAssertEqual(preview.sourceCount, 2)
        XCTAssertEqual(preview.checksumCount, 2)
        XCTAssertEqual(preview.sourcePreviewLabels, [
            "https://github.com/rust-lang/crates.io-index",
            "https://github.com/Lore-Hex/trusted-router-rs"
        ])
        XCTAssertEqual(preview.packagePreviewLabels, [
            "anyhow@1.0.86",
            "quillcode-rust-helper@0.1.0",
            "trusted-router@0.2.0"
        ])
        XCTAssertEqual(preview.byteSizeLabel, byteSizeLabel)
        XCTAssertEqual(preview.metadataLines, [
        ...
        try #"[package]\nname = "quillcode"\n"#.write(to: cargoToml, atomically: true, encoding: .utf8)
        XCTAssertNil(ToolArtifactState(value: cargoToml.path).cargoLockPreview)

        let remoteLock = ToolArtifactState(value: "https://example.com/Cargo.lock")
        XCTAssertNil(remoteLock.cargoLockPreview)
    }

    func testArtifactStateDerivesYarnLockfilePreviewMetadata() throws {
        let directory = try makeQuillCodeTestDirectory()
        let report = directory.appendingPathComponent("yarn.lock")
        let lockText = """
        ...
        let artifact = ToolArtifactState(value: report.path)
        let preview = try XCTUnwrap(artifact.yarnLockfilePreview)
        let byteSizeLabel = try XCTUnwrap(ToolArtifactByteSizeFormatter.label(for: XCTUnwrap(lockText.data(using: .utf8)).count))

        XCTAssertEqual(artifact.documentPreview?.kind, .data)
        XCTAssertEqual(artifact.documentPreview?.extensionLabel, "YARN-LOCK")
        XCTAssertEqual(preview.packageCount, 3)
        XCTAssertEqual(preview.versionedPackageCount, 3)
        XCTAssertEqual(preview.resolvedPackageCount, 3)
        ...
        let packageJSON = directory.appendingPathComponent("package.json")
        try #"{"dependencies":{"left-pad":"^1.3.0"}}"#.write(to: packageJSON, atomically: true, encoding: .utf8)
        XCTAssertNil(ToolArtifactState(value: packageJSON.path).yarnLockfilePreview)

        let remoteLock = ToolArtifactState(value: "https://example.com/yarn.lock")
