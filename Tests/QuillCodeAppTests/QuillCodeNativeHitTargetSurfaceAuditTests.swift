...
private extension QuillCodeNativeHitTargetSurfaceAuditTests {
    ...
    func assertRepresentativeContractSemantics(_ report: QuillCodeNativeHitTargetAuditReport) {
        ...
        XCTAssertEqual(contractsByID["onboarding.developer-key"]?.family, .settings)
        XCTAssertEqual(contractsByID["onboarding.developer-key"]?.testID, "quillcode-connect-developer-key")
        XCTAssertEqual(probesByContractID["onboarding.developer-key"]?.selectorKind, .testID)
