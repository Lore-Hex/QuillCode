...
test('mock harness covers the full pull request command family from the command palette', async ({ page }) => {
  ...
  await expect(page.getByTestId('pr-review-draft-summary-detail')).toHaveText('Approve review for current pull request');
  ...
  await page.getByTestId('pr-review-draft-inline-comment-body').first().fill('Cover this edited line.');
  ...
  await expect(page.getByTestId('pr-review-draft-submit')).toBeDisabled();
  await expect(page.getByTestId('pr-review-draft-summary')).toHaveAttribute('data-status', 'blocked');
  await expect(page.getByTestId('pr-review-draft-summary-detail')).toHaveText('Resolve required fields before submitting');
  ...
  await expect(page.getByTestId('pr-review-draft-summary')).toHaveAttribute('data-status', 'ready');
  await expect(page.getByTestId('pr-review-draft-summary-detail')).toHaveText('Request changes review for PR 123');
  ...
  await expect(inlineReviewCommentCard).toContainText('"body": "Cover this edited line."');
