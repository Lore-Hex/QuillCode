...
`./scripts/smoke.sh` runs the Swift tests, exercises the mock CLI in a temporary workspace, verifies that file creation works without dirtying the repo, and runs Playwright automatically when `E2E/playwright/node_modules` is present. Set `QUILLCODE_SMOKE_ARTIFACT_DIR` to preserve native smoke screenshots and a `deterministic-smoke-manifest.json` that records which deterministic sub-suites ran.
