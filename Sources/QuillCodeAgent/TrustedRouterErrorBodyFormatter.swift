...
enum TrustedRouterErrorBodyFormatter {
    static func streamingMessage(statusCode: Int, body: String) -> String {
        let suffix = normalizedBody(body)
    ...
    static func hint(forStatusCode statusCode: Int) -> String? {
        switch statusCode {
        case 401:
            return "Authentication failed — the API key is missing, invalid, or expired. "
                + "Sign in again (or refresh the developer override key) in Settings."
        case 403:
            return "Permission denied — the key is valid but not allowed to use this model or route. "
                + "Pick a different model or check the account's plan."
        default:
            return nil
        }
    }
    ...
    private static func normalizedBody(_ body: String) -> String {
        ...
        let end = singleLine.index(singleLine.startIndex, offsetBy: 1_000)
