...
struct GitWorktreeHandoffExecutor: Sendable {
    ...
    func handoff(sourceRoot: URL, destinationPath: String) -> ToolResult {
        ...
        do {
            let destinationRoot = try preflight.destination(
            ...
            do {
                ...
                let warning = cleanup.ok
                    ? ""
                    : "\nWarning: the destination is complete, but source cleanup was incomplete. Some task changes may remain in the source checkout. \(cleanup.error ?? "")"
                ...
                return ToolResult(
                    ok: true,
                    stdout: "Handed off task changes to \(destinationRoot.path). \(destinationSummary)\(warning)\n",
                    stderr: cleanup.ok ? "" : cleanup.error ?? "",
                    artifacts: [destinationRoot.path]
                )
            } catch {
                let rollback = destinationAlreadyMatched
                    ? ToolResult(ok: true)
                    : snapshotApplier.rollback(snapshot, at: destinationRoot)
                let rollbackDetail = rollback.ok
                    ? ""
                    : " Destination rollback also failed: \(rollback.error ?? "unknown error")"
    ...
    private func reversePatch(_ patchURL: URL, staged: Bool, sourceRoot: URL) -> ToolResult {
        ...
        guard result.ok else {
            return ToolResult(
                ok: false,
                error: "Could not remove the transferred \(staged ? "staged" : "unstaged") changes from the source: \(result.error ?? result.stderr)"
            )
        }
