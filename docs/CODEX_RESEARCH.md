...
- The latest Claude CLI UI pass called out three structural pressure points: top-bar pill overflow should collapse into clearer model/mode/context clusters, the sidebar plus Activity pane should not consume most of the window before the transcript gets space, and the HTML harness composer needed to become a multiline textarea so Playwright parity does not lie about native composer behavior. The composer now uses textarea semantics across SwiftUI, static HTML, and the Playwright harness, with Shift+Enter for newlines and Enter to send.
