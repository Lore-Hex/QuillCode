...
| UX | Top bar widget | Partial | Top-bar presentation state, HTML harness, SwiftUI shell, and macOS `MenuBarExtra` track thread/project/model/mode/status/Computer Use and expose quick New Chat, Open Project, Command Palette, Terminal, Browser, Settings, Stop All, and disabled Disconnect All affordances. Rich live progress, real cancellation, remote connection controls, and Linux tray adapter pending. |
