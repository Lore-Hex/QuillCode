...
- Native OS notification observation is a separate release-evidence contract from the app notifier boundary. Packaged scheduled-coworker smoke proves QuillCode emitted the task-specific notification report through the desktop notifier path, while `scripts/scheduled-notification-observation-smoke.sh` validates a redacted captured OS-banner/action observation only when a reviewer has real screenshot or Accessibility/Computer Use evidence. The validator requires the existing packaged scheduled-coworker manifest, visible task text, visible follow-up action, screenshot evidence, and rejects raw prompts or secrets, so scheduling rows cannot be marked covered by notification intent alone.
