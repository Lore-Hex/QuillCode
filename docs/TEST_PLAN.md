...
- add an SSH Remote from `Project: Add SSH Remote...`, complete `/ssh user@host:/path`, verify sidebar badge/path/top-bar context, verify local-only actions are disabled with a reason, and run `pwd` in the integrated terminal against the remote mock
...
- remote-pairing mock, SSH Remote registration mock, and SSH Remote terminal mock
