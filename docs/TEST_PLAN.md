...
- `./scripts/native-desktop-smoke.sh` runs the built `quill-code-desktop` executable, or an explicitly supplied desktop executable, in `--native-render-smoke` mode; exercises desktop chrome command routing for Command Palette, Keyboard Shortcuts, Settings, Terminal, and Browser; sends a real file-writing prompt through the desktop controller; verifies the created workspace file; verifies transcript message/tool/timeline counts; verifies the native SwiftUI hit-target audit report and every semantic target kind including the Thinking trace capsule when visible; renders the actual desktop root SwiftUI view to PNG; renders desktop chrome evidence to PNG; renders the same workspace surface to HTML; copies `report.json`, `workspace.png`, `chrome.png`, `workspace.html`, `stdout.log`, and `manifest.txt` into `QUILLCODE_NATIVE_DESKTOP_SMOKE_ARTIFACT_DIR` when set; and fails if the JSON report, tool result, chrome evidence, hit-target evidence, final answer, rendered result HTML, or rendered image is missing.
