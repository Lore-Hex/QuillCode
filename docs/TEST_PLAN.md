...
- `./scripts/build-macos-app.sh` creates a deterministic `QuillCode.app` bundle around the SwiftPM desktop product and validates its `Info.plist`. `./scripts/packaged-macos-smoke.sh` validates bundle metadata, runs the packaged app executable through the same native render-smoke path, then runs the bundle through `open -W -n QuillCode.app --args ...` so Launch Services argument passing and release entrypoints are covered before a release. It calls `scripts/native-click-probe-contracts.py compare` to compare direct-executable and Launch Services `nativeHitTargets.clickProbes` by selector, semantic kind/action, minimum target size, and normalized sample points, then writes `packaged-click-probes.json` with `launchServicesMatchesDirect`. It also calls `scripts/native-click-probe-contracts.py readiness` to reload the packaged artifact root and write `packaged-accessibility-readiness.json`, which records the report paths, contract IDs, required sample points, 40 pt native target floor, and the explicit next layer for live Accessibility/appshot frame sampling. Packaged coworker manifests preserve scheduled task evidence, multi-file artifact evidence, browser workflow evidence, and Computer Use setup/status evidence as `packaged-scheduled-coworker.json`, `packaged-multi-file-artifact.json`, `packaged-browser-workflow.json`, and `packaged-computer-use.json`; the browser manifest proves CRM-like and shared-sheet-like flows keep canonical `host.browser.type`/`click`/`script`/`inspect` arguments and live-DOM edited state across both packaged launch paths, while the Computer Use manifest proves setup/permission/refresh commands and recognized direct/Launch Services top-bar status labels survive packaging. The packaged live-window smoke also writes `window-report.json`/`window.png` from a real SwiftUI window, then calls `scripts/native-click-probe-contracts.py frames` to validate app/window identity, native click-probe contracts, top-bar identity, model/mode/status, disabled empty composer, `Chats` sidebar, core command IDs, every surfaced `commandID` backed by a `command.<id>` click-probe contract, default starter actions, image metadata, screenshot byte floor, live Accessibility frame samples, required sidebar/core frame IDs, 40 pt native floor, and exact center/interior sample coordinates. It writes `packaged-accessibility-frames.json` so release artifacts expose live frame evidence plus `windowCommandContractCount`/`windowCommandContractIDs` without requiring reviewers to dig through `window-report.json`. When `QUILLCODE_PACKAGED_MACOS_SMOKE_ARTIFACT_DIR` is set, it preserves the direct-executable and Launch Services smoke screenshots/reports, the click-probe manifest, the Accessibility readiness manifest, the packaged coworker manifests, the live Accessibility frame manifest, live-window report/screenshot, and the package `Info.plist`.
