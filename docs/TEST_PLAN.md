...
- add an SSH Remote from `Project: Add SSH Remote...`, complete `/ssh user@host:/path`, verify sidebar badge/path/top-bar context, verify local-only actions are disabled with a reason, run `pwd` in the integrated terminal against the remote mock, verify terminal and tool-card execution-context chips/rails say `SSH Remote`, and verify chat-driven `whoami` plus remote file-creation requests use `host.shell.run` instead of local file tools
...
- remote-pairing mock, SSH Remote registration mock, SSH Remote terminal mock, and SSH Remote chat shell mock
