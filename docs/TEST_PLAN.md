...
- `./scripts/build-macos-app.sh` creates a deterministic `QuillCode.app` bundle around the SwiftPM desktop product and validates its `Info.plist`. `./scripts/packaged-macos-smoke.sh` validates bundle metadata and runs the packaged app executable through the same native render-smoke path, catching bundle-entrypoint drift before a release.
