...
- Project connection parsing, typed local/SSH Remote persistence, legacy project decoding, remote display labels, destination-only address validation, and immutable remote-path replacement.
- OpenSSH config tokenization, concrete-alias filtering, quoted and equals-form directives, bounded recursive Include discovery, glob character classes, hidden-file behavior, missing includes, cycles, symlinks, file/depth/byte/alias limits, effective `ssh -G` resolution, and remote-folder probe marker/path/error handling.
...
- Automation recurrence, deterministic one-off calendar phrase parsing, event-source metadata persistence, file-change event-source resolution, project-relative path containment, remote-relative rejection, trigger-aware due selection, monitor draft trigger recording, off-main event polling, four-source concurrency bounds, deterministic result ordering, prompt cancellation, stale-snapshot rejection, zero-limit short-circuiting, and immediate first ticker execution.
...
- Patch parser, diff parser, file/line/range review comments, approval Run/Edit/Skip planning, delegated-worker approval request validation, exact held-call resume, duplicate-decision rejection, Auto reviewer JSON, reviewer-route telemetry, permission-rule telemetry preservation, redacted `scripts/safety-reviewer-calibration-smoke.sh` manifest validation, recurring `scripts/safety-reviewer-calibration-rollup.sh` JSON plus Markdown calibration summary validation, and sandbox policy.
...
- MCP progress tokens and notifications: generated and caller-supplied string/integer tokens, structured metadata preservation, exact-token matching, strict monotonic finite values, positive optional totals, Boolean rejection, bounded/sanitized messages, update-count limits, transcript coalescing, exact tool-card targeting, terminal cleanup, and cancellation repair. App-server integration additionally proves exact `threadId`/`turnId`/`itemId` correlation, start-progress-complete ordering, suppression of stale/non-MCP/message-free updates that cannot satisfy the Codex wire schema, notification opt-out routing, and unchanged standalone tool-call responses.
...
- App-server sessions with fake streaming/blocking/scripted LLMs: thread lifecycle, response-before-
  ...
  file approval accept/decline/error behavior, EOF cleanup, and durable transcript recovery. Direct
  user-shell coverage must prove exact empty-input errors, configured-shell selection, standalone and
  concurrent lifecycle, active ordinary/review/compaction turn reuse, model feedback, parent completion
  ordering under a deliberately slow command, interruption, rollback, restart-safe empty history turns,
  and no command-item leakage into read/list/fork projections.
...
- Desktop automation polling with blocking fake sources, proving the main thread remains available,
  cancellation returns before a synchronous adapter unblocks, no more than four adapters poll at
  once, completion order cannot reorder triggers, a mid-poll pause suppresses the stale trigger, and
  overdue work runs before the ticker's first sleep interval.
...
- run shell
- submit a mock MCP turn through the composer, hold the tool card at determinate progress, verify accessible label/value and stable percentage layout, observe immediate Done cleanup and final answer, and render an indeterminate call without a fabricated percentage
...
- `QuillCodeDesktopProgressRefreshSchedulerTests` sends a 10,000-signal synchronous burst through the native presentation scheduler and requires one pending rebuild, latest-action publication, immediate terminal flush, cancellation cleanup, and zero scheduler retention. `QuillCodeDesktopConcurrentChatTests/testDesktopComposerSeparatesImmediateAndStreamingRefreshes` proves model fragments use that bounded path while send start and finish remain immediate.
- Terminal retention coverage floods the PTY and workspace model beyond their byte/line budgets, verifies newest-tail and valid UTF-8 preservation, bounds pending process events while retaining `.finished`, consumes DEC mouse/keyboard modes from the complete raw stream, caps oversized final results and initial history, preserves stopped output, and proves the desktop publication callback exposes Running output before completion. The native render smoke also drives a 40,000-line command through `QuillCodeDesktopController`, requires a live frame before completion, verifies bounded truncated final output, and restores an empty hidden terminal afterward; packaged direct-executable and Launch Services smoke inherit the same gate.
...
- `./scripts/build-macos-app.sh` creates a deterministic `QuillCode.app` bundle around the SwiftPM desktop product and validates its `Info.plist`. `./scripts/packaged-macos-smoke.sh` validates bundle metadata, runs the packaged app executable through the same native render-smoke path, then runs the bundle through `open -W -n QuillCode.app --args ...` so Launch Services argument passing and release entrypoints are covered before a release. It calls `scripts/native-click-probe-contracts.py compare` to compare direct-executable and Launch Services `nativeHitTargets.clickProbes` by selector, semantic kind/action, minimum target size, and normalized sample points, then writes `packaged-click-probes.json` with `launchServicesMatchesDirect`. It also calls `scripts/native-click-probe-contracts.py readiness` to reload the packaged artifact root and write `packaged-accessibility-readiness.json`, which records the report paths, contract IDs, required sample points, 40 pt native target floor, and the explicit next layer for live Accessibility/appshot frame sampling. Packaged coworker manifests preserve scheduled task evidence, multi-file artifact evidence, browser workflow evidence including authenticated session state, Computer Use setup/status evidence, and Computer Use action evidence as `packaged-scheduled-coworker.json`, `packaged-multi-file-artifact.json`, `packaged-browser-workflow.json`, `packaged-computer-use.json`, and `packaged-computer-use-action.json`; the browser manifest proves CRM-like, shared-sheet-like, and authenticated-session flows keep canonical `host.browser.type`/`click`/`script`/`inspect` arguments and live-DOM edited state across both packaged launch paths, the Computer Use setup manifest proves setup/permission/refresh commands and recognized direct/Launch Services top-bar status labels survive packaging, and the Computer Use action manifest proves `host.computer.screenshot`/`click`/`type`/`scroll`/`move`/`key` execute with non-empty canonical arguments through both packaged launch paths. The packaged live-window smoke also writes `window-report.json`/`window.png` from a real SwiftUI window, then calls `scripts/native-click-probe-contracts.py frames` to validate app/window identity, native click-probe contracts, top-bar identity, model/mode/status, disabled empty composer, `Chats` sidebar, core command IDs, every surfaced `commandID` backed by a `command.<id>` click-probe contract, default starter actions, image metadata, screenshot byte floor, live Accessibility frame samples, required sidebar/core frame IDs, 40 pt native floor, and exact center/interior sample coordinates. It writes `packaged-accessibility-frames.json` so release artifacts expose live frame evidence plus `windowCommandContractCount`/`windowCommandContractIDs` without requiring reviewers to dig through `window-report.json`. When `QUILLCODE_PACKAGED_MACOS_SMOKE_ARTIFACT_DIR` is set, it preserves the direct-executable and Launch Services smoke screenshots/reports, the click-probe manifest, the Accessibility readiness manifest, the packaged coworker manifests, the live Accessibility frame manifest, live-window report/screenshot, and the package `Info.plist`.
- `./scripts/live-saas-template.sh <output.json> <catalog-task-id> [catalog-task-id ...]` writes a row-linked evidence skeleton for optional credentialed/live-service captures, and `./scripts/live-saas-smoke.sh <evidence.json> [manifest.json]` is the fail-closed validation gate for coworker rows that depend on signed-in SaaS state. The validator calls `scripts/native-click-probe-contracts.py live-saas`, requires exact spreadsheet `catalogTaskIDs`, signed-in account state, HTTPS URL, `host.browser.open` plus `host.browser.inspect`, at least one browser or Computer Use action, `Live DOM snapshot` evidence, and Computer Use screenshot proof when Computer Use is present; it rejects common captured secrets before writing `live-saas-manifest.json`. `./scripts/live-app-computer-use-template.sh <output.json> <catalog-task-id> [catalog-task-id ...]` writes the row-linked skeleton for optional arbitrary foreground-app captures, and `./scripts/live-app-computer-use-smoke.sh <evidence.json> [manifest.json]` validates completed local-app Computer Use evidence; it requires exact spreadsheet `catalogTaskIDs`, matching app/foreground names, `host.computer.screenshot`, at least one click/type/scroll/move/key action, screenshot/appshot proof, visible before/after state change, completed result evidence, and no raw prompts or captured secrets. `./scripts/scheduled-notification-observation-template.sh <output.json> <catalog-task-id> [catalog-task-id ...]` writes the row-linked skeleton for optional packaged native-notification observation captures, and `./scripts/scheduled-notification-observation-smoke.sh <evidence.json> [manifest.json]` validates the completed evidence; it requires exact spreadsheet `catalogTaskIDs`, a redacted observation tied to `packaged-scheduled-coworker.json`, visible task-specific notification text, an observed Open follow-up action, screenshot evidence, and no raw prompts or captured secrets. `scripts/native-click-probe-contracts.py coworker-catalog ... --output coworker-coverage.json` rolls validated live SaaS, live app Computer Use, scheduled-notification, and packaged one-turn coworker manifests into row-level `provenTaskIDs`, `pendingTaskIDs`, `evidenceByTaskID`, and a fail-closed audit across the checked-in catalog snapshot (currently rows 1-310). These optional live/OS gates are intentionally not part of default CI because third-party account sessions, local app data, and OS notification banners are private and brittle.
- Packaged live-window smoke must also prove selected controls activate through real macOS Accessibility. The `frames` validator requires `accessibilityActivation.liveAccessibilityActivation == "ax-press-sampled"`, verifies the composer model picker, New Chat, Search, Settings, Automations, Extensions, Memories, and Activity were AX-pressed, and preserves their before/after state summaries in `packaged-accessibility-frames.json`. Search must expose its identified field, focus it automatically, accept a reversible AXValue text edit, and clear it. The model picker must focus its identified search field, accept and clear `Prometheus` through AXValue, and expose the identified `trustedrouter/fusion` row as `Prometheus 1.0` without changing the selected model. New Chat must create and select exactly one chat, focus the identified composer, accept and clear reversible AXValue text, then remove only the temporary chat and restore the prior selection/draft. Settings must expose Account and its API field, switch to General through AXPress, render Notifications, return to Account, and dismiss through the identified Close control; state-only evidence is rejected. Automations must expose its identified title and Create control, then dismiss through its identified Close control with AXPress; state-only evidence is rejected. Extensions and Memories must each expose their identified title and Add control, then dismiss through their identified Close control with AXPress; state-only evidence is rejected. Activity must expose its identified title and task summary, dismiss through its identified Close control, and restore at least 240 points of composer width; state-only or dismissal-only evidence is rejected. Hidden compact-layout commands may fall back to titled native menu items only after identified workspace controls fail resolution. Workspace-replacing contracts run after transient-surface contracts and each press resolves a fresh AX target so smoke never relies on stale native elements.
- Every packaged Accessibility activation records physical footprint and thread count immediately before AXPress and after the presented hierarchy stabilizes. The report names the peak live surface and maximum growth, recomputes each delta, and fails on forged evidence, any presented footprint above 128 MiB, or any baseline/presented thread count above 32. This transient gate complements the settled two-sweep performance budget.
...
- Packaged performance smoke must run the reversible native Accessibility interaction sweep twice in every fresh process. It records initial-window, first-settled, repeated-settled, and two-second idle resource snapshots; recomputes CPU/memory/thread deltas from raw values; and fails if any sample exceeds 128 MiB or 32 threads, first-pass retained growth exceeds 64 MiB, the repeated pass adds more than 16 MiB or two retained threads above the higher prior settled count, or settled idle exceeds 5% process CPU, 8 MiB retained growth, or two additional threads. Schema-7 evidence keeps the raw second-minus-first thread delta separately and rejects a forged retained high-water delta.
- Download publication must package arm64 on `macos-15` and x86_64 on `macos-15-intel`, first proving
  `uname -m` matches the matrix architecture. Release policy must require successful exact-main CI,
  stop missing or terminal unsuccessful runs at the initial 30-minute boundary, grant at most one
  additional 30-minute grace to a currently active run, preserve eligibility through transient
  GitHub query loss only after a prior active observation, and stop at the combined one-hour bound. Each runner
  executes the packaged performance smoke;
...
- Offline and public release verification must require one DMG, app ZIP, CLI archive, build-info file,
  ...
  even after fixture digests, checksums, manifest metadata, and release API metadata are regenerated.
...
## Cheap Cross-Domain Live Evals

- `./scripts/cheap-agentic-evals.py` runs the versioned 12-case catalog in
  `docs/cheap-agentic-eval-catalog.json` through the production CLI and exact
  `deepseek/deepseek-v4-flash-0731` task-model route. The default two trials are hard-capped at 24
  paid invocations. Cases use synthetic defensive-security, non-clinical biology, AI-data, evaluator
  robustness, and multi-file fixtures; graders inspect exact JSON/text, protected-input hashes, or
  allowlisted local Python tests. Each case gets an ephemeral home and isolated workspace. The
  manifest records per-trial category scores, score-vector agreement, token usage, and secret-free
