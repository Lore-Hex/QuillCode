...
- Workspace activity reduction from thread events, tool cards, instructions, memories, artifacts, latest assistant answers, deterministic handoff summaries, and collapsible shared section state.
...
- open the Activity pane and verify the current task, recent events, tools, sources, artifacts, latest answer, handoff summary, and collapsible section state are reconstructed from the same transcript state
