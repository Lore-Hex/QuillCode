...
- `QuillCodeDesktopRenderedSmokeTests/testRenderedWorkspaceShowsRealWorldActionResult` runs a real file-writing prompt through the desktop controller, renders the SwiftUI workspace shell and the native message/tool-card result components to pixels, and fails if the app chrome or action-result UI is blank, monochrome, missing accent controls, or missing the real file side effect.
- `./scripts/native-desktop-smoke.sh` runs the built `quill-code-desktop` executable, or an explicitly supplied desktop executable, in `--native-render-smoke` mode; exercises desktop chrome command routing for Command Palette, Keyboard Shortcuts, Settings, Terminal, and Browser; sends a real file-writing prompt through the desktop controller; verifies the created workspace file; verifies transcript message/tool/timeline counts; verifies the native SwiftUI hit-target audit report and every semantic target kind including the Thinking trace capsule when visible; renders the actual desktop root SwiftUI view to PNG; renders a focused native result evidence image with the prompt, completed tool, final answer, and artifact name; renders desktop chrome evidence to PNG; renders the same workspace surface to HTML; copies `report.json`, `workspace.png`, `result.png`, `chrome.png`, `workspace.html`, `stdout.log`, and `manifest.txt` into `QUILLCODE_NATIVE_DESKTOP_SMOKE_ARTIFACT_DIR` when set; and fails if the JSON report, tool result, result evidence, chrome evidence, hit-target evidence, final answer, rendered result HTML, or rendered image is missing.
...
- `./scripts/smoke.sh` runs Swift tests, mock CLI `run whoami`, natural diagnostic prompts (`whoami?`, disk usage, OpenClaw discovery), mock CLI file creation, a local `file://` download into a requested workspace path, live-mode missing-key error readability, native desktop executable render smoke, packaged macOS app smoke on Darwin, and Playwright E2E when local node modules are installed. CI sets `QUILLCODE_SMOKE_ARTIFACT_DIR` and uploads the native UI smoke screenshots/reports as a short-retention artifact on both success and failure so PR reviewers can inspect the actual rendered workspace and chrome.
- `./scripts/live-tr-smoke.sh` is the opt-in real TrustedRouter smoke. It reads `QUILLCODE_API_KEY`, `TRUSTEDROUTER_API_KEY`, or `~/.quill.code.keyfile`, defaults to the cheap `deepseekv4flash` alias, runs through `quill-code --live`, and fails on empty shell arguments, passive “I’ll run/check/do/download/create/write...” answers, missing command output, unreadable live errors, natural disk-usage prompts without concrete output, model-planned file writes that do not actually create the requested temp-workspace files, OpenClaw/device-discovery prompts without concrete output, public-page download prompts that do not create a nonempty requested file, or persisted transcripts that lack nonempty queued tool calls and successful completed tool results. Each scenario writes a `live-smoke-report.jsonl` record with model, base URL, duration, stdout/stderr byte counts, artifact paths, and prompt; failures print the current scenario, prompt, stdout/stderr tails, and the report summary so real-provider regressions are diagnosable from one terminal log.
- `./scripts/real-world-smoke.sh` is the release-candidate wrapper. It always runs deterministic `./scripts/smoke.sh`, then runs live TrustedRouter smoke when a key is available. Set `QUILLCODE_REQUIRE_LIVE_SMOKE=1` when validating parser, prompt, safety-review, model-routing, or runtime changes so a missing key fails the release smoke instead of silently skipping the real model path.
...
- Live TrustedRouter smoke is not required for normal PRs. Run `./scripts/real-world-smoke.sh` before releases, model-prompt changes, parser changes, runtime changes, or safety-review changes; use `QUILLCODE_REQUIRE_LIVE_SMOKE=1` when the live model path must be proven.
