...
- add an SSH Remote from `Project: Add SSH Remote...`, complete `/ssh user@host:/path`, verify sidebar badge/path/top-bar context, refresh remote context from mock AGENTS/rules/memories, verify mutating git/worktree/apply-patch actions are disabled with a reason, run `pwd` in the integrated terminal against the remote mock, run read-only git status/diff from the command palette, verify terminal and tool-card execution-context chips/rails say `SSH Remote`, and verify chat-driven `whoami` uses `host.shell.run` while remote file read/write requests use `host.file.read`/`host.file.write` over SSH instead of local file tools
