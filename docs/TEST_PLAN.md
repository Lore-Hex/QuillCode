...
- `./scripts/live-saas-smoke.sh <evidence.json> [manifest.json]` is the optional credentialed/live-service gate for coworker rows that depend on signed-in SaaS state. It calls `scripts/native-click-probe-contracts.py live-saas`, requires signed-in account state, HTTPS URL, `host.browser.open` plus `host.browser.inspect`, at least one browser or Computer Use action, `Live DOM snapshot` evidence, and Computer Use screenshot proof when Computer Use is present; it rejects common captured secrets before writing `live-saas-manifest.json`. This gate is intentionally not part of default CI because third-party account sessions are private and brittle.
