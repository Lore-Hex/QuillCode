...
- Packaged performance smoke must run the reversible native Accessibility interaction sweep twice in every fresh process. It records initial-window, first-settled, repeated-settled, and two-second idle resource snapshots; recomputes CPU/memory/thread deltas from raw values; and fails if any sample exceeds 128 MiB or 32 threads, first-pass retained growth exceeds 64 MiB, the repeated pass adds more than 12 MiB or two threads, or settled idle exceeds 5% process CPU, 8 MiB retained growth, or two additional threads.
