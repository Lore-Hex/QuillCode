...
- `./scripts/native-desktop-smoke.sh` runs the built `quill-code-desktop` executable in explicit `--native-render-smoke` mode, sends a real file-writing prompt through the desktop controller, verifies the created workspace file, verifies transcript message/tool/timeline counts, renders the actual desktop root SwiftUI view to PNG, renders the same workspace surface to HTML, and fails if the JSON report, tool result, final answer, rendered result HTML, or rendered image is missing.
