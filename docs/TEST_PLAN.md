...
- Workspace activity reduction from thread events, tool cards, instructions, memories, artifacts, latest assistant answers, and collapsible shared section state.
...
- open the Activity pane and verify the current task, recent events, tools, sources, artifacts, latest answer, and collapsible section state are reconstructed from the same transcript state
