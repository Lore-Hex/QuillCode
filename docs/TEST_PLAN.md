...
- `./scripts/native-desktop-smoke.sh` runs the built `quill-code-desktop` executable in explicit `--native-render-smoke` mode, exercises desktop chrome command routing for Command Palette, Keyboard Shortcuts, Settings, Terminal, and Browser, sends a real file-writing prompt through the desktop controller, verifies the created workspace file, verifies transcript message/tool/timeline counts, renders the actual desktop root SwiftUI view to PNG, renders desktop chrome evidence to PNG, renders the same workspace surface to HTML, and fails if the JSON report, tool result, chrome evidence, final answer, rendered result HTML, or rendered image is missing.
