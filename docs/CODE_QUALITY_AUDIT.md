...
| Before | After |
| --- | --- |
| `containsDownloadSegment` (split on `&&`) confirmed a download existed; the rest of the guard validated only the *first* `--output` and "no pipe". Nothing constrained the other segments — **nor the curl segment's own flags**. Auto-approved on a bare "download" intent: `… && rm -rf ~/old` / `; git reset --hard`; a redirect `curl --output dl/p … >> ~/.bashrc`; a second `--output '/etc/cron.d/evil'`; `curl -K /tmp/evil.cfg`; a double-quoted `curl "…/$(rm -rf ~)"`; and a whole family of curl flags that write a server-controlled file to an arbitrary path — `-D`/`--dump-header`, `-c`/`--cookie-jar`, `--trace-ascii`, `--output-dir /etc`, `-O` (the last reached because the lowercased command made `-O` match the `-o` regex). | `isScopedToDownload` now: (1) neutralizes single-quoted spans (literal), then rejects **any active shell metacharacter** (`>` `<` `` ` `` `$` `"` `\` `*` `?` `(` `)` `{` `}` `@`) anywhere — closing redirects, substitution, expansion, globs, subshells, and `@file` reads; (2) splits on **every** operator and requires each segment to be the download or a safe companion (`cd` excluded); (3) gates the curl invocation with an **allowlist of safe flags** (case-sensitive, on the original command) — anything else (`-O`, `-c`, `-D`, `--trace*`, `--output-dir`, `-T`, `-K`, …) drops to human approval. `wget` is no longer auto-approved (its flag set has too many file traps). Every `--output`/`-o` target is still validated workspace-relative. |

The architectural fix is the **allowlist**: `allOutputTargetsWorkspaceRelative` was allow-by-omission — it could only validate the flags it enumerated, while curl honors hundreds. Accepting only known-safe flags fails safe (an unrecognized flag → human approval) instead of failing open. Two more closes followed: `--proto`/`--proto-default` were dropped from the allowlist (`--proto-default file` turns a schemeless arg into a `file://` read of an arbitrary local file), and a `file://` URL in the command must now be one the **user explicitly named** — otherwise a "download from `<host>`" intent whose substring host gate is satisfied by an `-e` referer could be turned into `curl -e 'https://host' --output x 'file:///etc/passwd'`. Bundled boolean short flags (`-fsSL`, idiomatic) are accepted so the common case still auto-approves.

Verification — **non-vacuous**, proven by reverting only the source guard at each stage; every bypass above returns `approve` on the immediately-prior version and is blocked after. The legitimate single-quoted approve cases (`mkdir && curl --output 'x' '…' && ls`, the `-fsSL` bundle, a user-named `file://`) still pass. `swift test` **1904** green · native smoke clean.

Adversarial-review note: this carve-out took **three** review rounds — round 1 caught the curl-segment gaps (redirect, multi-output, `-K`, double-quote); round 2 caught the file-writing-flag family (`-D`/`-c`/`--output-dir`/`-O`) and the allow-by-omission root cause (→ allowlist rewrite); round 3 caught a flag I had *wrongly* allowlisted (`--proto-default`, an arbitrary-file-read). My own passes caught the `cd`-relocation, backslash-desync, `@file`-read, and the `file://`-via-referer read. **Lesson: a narrow auto-approve carve-out must constrain its *privileged* tool (curl) by allowlist, not denylist — and even the allowlist needs every member's semantics understood (a "protocol" flag was actually a local-read primitive).**
...
## 2026-06-29 Persisted Sidebar Saved-Search Click-Target Pass
