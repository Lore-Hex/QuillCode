...
- Richer non-image artifact previews, plugin/skill install and execution lifecycle, richer MCP per-tool schemas, streaming beyond the generic verified-server call path, and richer prompt/resource UX. MCP resource reads and prompt gets now run through allowlisted Ready-server agent tools, building on the tested discovery UI contract. Autonomous memory writes/richer redaction/Chronicle jobs, memory editing, and automations.
