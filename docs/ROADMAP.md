...
- Native smoke tests.
- Current status: testable workspace state model, persisted config/thread/project bootstrap, HTML surface contract, Playwright harness, and SwiftUI desktop shell exist for project rail, sidebar, composer, top bar, transcript, tool-card presentation, git diff review summaries, slash commands, grouped model selection, mode switching, and native developer settings. The desktop app seeds the current working directory as the initial project and runs tools from the selected project path.
...
- Review pane controls and hunk actions, apply patch, integrated terminal, worktrees, local env actions.
