...
- Native smoke tests.
- Current status: testable workspace state model, persisted config/thread/project bootstrap, bounded broad-to-specific project instruction loading from root and nested `AGENTS.md`/`.quillcode` rules, bounded project-local extension manifest discovery for plugins, skills, and MCP servers, HTML surface contract, Playwright harness, smoke script, and SwiftUI desktop shell exist for project rail, visible New chat/Search/Open project/Browser/Terminal/Extensions sidebar actions, desktop folder picking, thread pin/archive controls, pinned/recent chat grouping, thread search, grouped/ranked command palette with keyboard navigation, registry-backed primary keyboard shortcuts, local environment action commands from `.quillcode/actions` and `.quillcode/local-env`, browser preview state and comments, composer, in-app top bar, native macOS menu-bar widget, chronological event-driven transcript, estimated context pressure banner with New thread/Fork from last actions, incremental agent-run progress, tool-card presentation with human final-answer bubbles, artifact preview chips, and collapsed raw details for successful tools, integrated workspace terminal command history, git diff review summaries with post-patch refresh, file/hunk Stage/Restore controls, and file-scoped review notes, git worktree command-palette actions, create/remove dialogs, and create-time worktree project/thread handoff, slash commands for core workspace actions and local environment scripts, searchable grouped model selection, mode switching, and native developer settings. The desktop app seeds the current working directory as the initial project and runs tools from the selected project path.
...
- Rich artifact previews, plugin/skill install and execution lifecycle, MCP server process management, memories, and automations.
